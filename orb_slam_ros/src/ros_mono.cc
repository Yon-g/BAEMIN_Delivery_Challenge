...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_img = node_handler.subscribe("/image_jpeg/compressed", 1, &ImageGrabber::GrabImage, &igb);
...
void ImageGrabber::GrabImage(const sensor_msgs::CompressedImageConstPtr& msg)
{
    ...
    try
    {
        std::vector<unsigned char> compressed_data = msg->data;
        ...
        // cv_bridge::CvImage로 수동 변환
        cv_ptr = boost::make_shared<cv_bridge::CvImage>();
        cv_ptr->header = msg->header;
        cv_ptr->encoding = sensor_msgs::image_encodings::BGR8;  // 적절한 인코딩 설정
        cv_ptr->image = image;
