...
void Tracking::Track()
{
    ...
    else
    {
        ...
        if(bOK)
        {
            mState = OK;
            // 특징점 수와 트래킹 상태를 출력
            std::cout << "Tracking state: " << mState <<  " (OK)" << std::endl;
            std::cout << "Number of keypoints: " << mCurrentFrame.N << std::endl;
            std::cout << "Number of matched MapPoints: " << mCurrentFrame.mvpMapPoints.size() << std::endl;
        }
        else if (mState == OK)
        {
            if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
            {
                ...
                // 트래킹 실패 시에도 특징점 정보를 출력
                std::cout << "Tracking state: " << mState <<  " (RECENTLY_LOST)" << std::endl;
                std::cout << "Number of keypoints: " << mCurrentFrame.N << std::endl;
                std::cout << "Number of matched MapPoints: " << mCurrentFrame.mvpMapPoints.size() << std::endl;
            }
            else
...
bool Tracking::TrackLocalMap()
{
    ...
    // MapPoints와의 매칭 후 특징점 수 출력
    int nmatches = Optimizer::PoseOptimization(&mCurrentFrame);

    // 매칭된 MapPoints의 수 출력
    std::cout << "Number of matched MapPoints in TrackLocalMap: " << nmatches << std::endl;

    // 최적화 후의 특징점 수 출력
    std::cout << "Number of keypoints after optimization: " << mCurrentFrame.N << std::endl;

    // 트래킹 상태 출력
    std::cout << "Tracking state in TrackLocalMap: " << mState << std::endl;
